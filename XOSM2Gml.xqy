...
{
 if ($document instance of xs:string) then $document 
 else
 if ($document instance of xs:integer) then $document
 else 
  if ($document instance of xs:double or $document instance of xs:decimal) then $document 
 else 
  ...
  return 
   if (exists($document1[name(.) = 'osm']))
    then ($document[name(.) = 'node']) union (($document//way) union ($document//node)) 
      ...
      else if ($document = 0) then 0 else  "No Result" 
};

declare function xosm_gml:_result2OsmV2($document)
{
 if ($document instance of xs:string) then 
 <osm comment="CAUTION!! This is not a valid OSM document">
    <text>{$document}</text>
    <errorType/>
    <errorDescription/>
 </osm> 
 else
 if ($document instance of xs:integer) then 
  <osm comment="CAUTION!! This is not a valid OSM document">
    <text>{$document}</text>
    <errorType/>
    <errorDescription/>
  ...
  if ($document instance of xs:double or $document instance of xs:decimal) then 
   <osm comment="CAUTION!! This is not a valid OSM document">
    <text>{$document}</text>
    <errorType/>
    <errorDescription/>
 ...
 if ($document[1][name(.) = 'social']) then $document
 else 
 if ($document[1][name(.) = 'oneway']) 
 then 
 <osm version='0.6' upload='true' generator='JOSM'>
  ...
  return 
   if (exists($document1[name(.) = 'osm']))
    then ($document[name(.) = 'node']) union (($document//way) union ($document//node)) 
      ...
      else if ($document = 0) then 0 else 
      <osm comment="CAUTION!! This is not a valid OSM document">
        <text>No Result</text>
        <errorType/>
        <errorDescription/>
