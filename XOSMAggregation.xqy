...
declare function xosm_ag:topologicalCountG($document as node()*, $oneway as node(), 
$topologicalRelation)
{
 count(fn:filter($document,
       function($onewayRef){$topologicalRelation($onewayRef,$oneway)})) 
};
...
declare function xosm_ag:metricMinG($document as node()*, $metricOperator)
{
  let $list := xosm_ag:metricListG($document,$metricOperator) ,
     $minValue := fn:min(data($list/tag[@k="metric"]/@v)) 
 return 
     fn:filter($list, function($oneway){xosm_kw:searchTag($oneway,"metric",$minValue)})
};
...
declare function xosm_ag:metricMaxG($document as node()*, $metricOperator)
{
  let $list := xosm_ag:metricListG($document,$metricOperator) ,
     $minValue := fn:max(data($list/tag[@k="metric"]/@v)) 
 return 
     fn:filter($list, function($oneway){xosm_kw:searchTag($oneway,"metric",$minValue)})
};
...
declare function xosm_ag:metricSumG($document as node()*,$metricOperator)
{
  let $list :=  xosm_ag:metricListG($document,$metricOperator)
  return
  fn:sum(fn:for-each($list,
         function($oneway){xosm_kw:getTagValue($oneway,"metric")})) 
};
...
declare function xosm_ag:minG($document as node()*,$metricOperator)
{
  let $list := xosm_ag:metricListG($document,$metricOperator) ,
     $minValue := fn:min(data($list/tag[@k="metric"]/@v)) 
 return 
...
declare function xosm_ag:maxG($document as node()*,$metricOperator)
{
  let $list := xosm_ag:metricListG($document,$metricOperator) ,
     $maxValue := fn:max(data($list/tag[@k="metric"]/@v)) 
 return 
     $maxValue
};
...
declare function xosm_ag:metricDistance($document as node()*, $street as node())
{
  ...
  if ($each[@type="way"]) then xosm_gml:_osm2GmlLine($each)
...
declare function xosm_ag:metricListG($document as node()*, $metricOperator)
{
  fn:for-each($document, function($oneway)
     {xosm_ag:addAggregationTag($oneway,"metric",xs:string($metricOperator($oneway)))})
};
...
declare function xosm_ag:metricAvgG($document as node()*,$metricOperator)
{
 let $list := xosm_ag:metricListG($document,$metricOperator)
 return
...
declare function xosm_ag:metricBottomCountG($document as node()*, $metricOperator, $k as xs:integer)
{
  let $list := xosm_ag:metricListG($document,$metricOperator)
  return fn:subsequence(fn:sort($list, 
         function($oneway){xosm_kw:getTagValue($oneway,"metric")}),1,$k)             
};
...
declare function xosm_ag:metricTopCountG($document as node()*, $metricOperator, $k as xs:integer)
{
  let $list := xosm_ag:metricListG($document,$metricOperator)
  return fn:subsequence(fn:sort($list, 
         function($oneway){-xosm_kw:getTagValue($oneway,"metric")}),1,$k)             
...
declare function xosm_ag:metricMedian($document as node()*, $metricOperator as xs:string)
{
  ...
  return 
    if ($count mod 2 != 0) then
      $orderedList[xs:integer($count div 2)+1]/tag[@k=$metricOperator]/@v
    else 
      ($orderedList[$count div 2]/tag[@k=$metricOperator]/@v 
      + $orderedList[($count div 2) + 1]/tag[@k=$metricOperator]/@v) div 2
};

declare function xosm_ag:metricMedianG($document as node()*, $metricOperator)
{
  let $list := xosm_ag:metricListG($document,$metricOperator),
      $orderedList := 
          fn:sort($list,function($oneway){xosm_kw:getTagValue($oneway,"metric")}),
      $count := count($orderedList)
  return 
    if ($count mod 2 != 0) then
      xs:double(data($orderedList[xs:integer($count div 2)+1]/tag[@k="metric"]/@v))
    else 
      xs:double(data($orderedList[$count div 2]/tag[@k="metric"]/@v 
      + $orderedList[($count div 2) + 1]/tag[@k="metric"]/@v) div 2)
};

declare function xosm_ag:metricModeG($document as node()*,$metricOperator)
{
  let $list := xosm_ag:metricListG($document,$metricOperator)
  return
  let $com :=
  (for $o in $list
  let $v := $o/tag[@k="metric"]/@v
  group by $v
  order by count($o) descending
  return <a><value>{$v}</value><count>{count($o)}</count></a>)
  return ($com/value)[1]/text()
...
declare function xosm_ag:metricRankG($document as node()*, 
$metricOperator, $k as xs:integer)
{
  let $list := xosm_ag:metricListG($document,$metricOperator)
  return fn:sort($list,
         function($oneway){- xosm_kw:getTagValue($oneway,'metric')})[$k]
};
...
declare function xosm_ag:metricRangeG($document as node()*,
   $metricOperator)
{ 
   let $list := xosm_ag:metricListG($document,$metricOperator)
  let $list2 := fn:for-each($list,
               function($oneway){xosm_kw:getTagValue($oneway,'metric')}),
       $maxValue := fn:max($list2),
       $minValue := fn:min($list2)       
  return 
       $maxValue - $minValue
     };